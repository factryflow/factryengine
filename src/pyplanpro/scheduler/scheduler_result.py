...
class SchedulerResult():
    ...
    def plot_resource_plan(self) -> None:
        df = self.to_dataframe()
            # Sort dataframe by assigned_resource_id and task_start to visualize in order
        df = df.sort_values(['assigned_resource_id', 'task_start'])

        # Create a color dictionary for each unique resource for distinction in the plot
        resources = df['assigned_resource_id'].unique()
        colors = sns.color_palette("deep", len(resources))  # Using seaborn "dark" color palette
        ...
        for resource_id, group_df in df.groupby('assigned_resource_id'):
            for task in group_df.itertuples():
                plt.barh(resource_id, 
                        left=task.task_start, 
                        width=task.task_end - task.task_start, 
                        color=color_dict[resource_id],
                        edgecolor='black')
                plt.text(x=(task.task_start + task.task_end)/2,  # x position, in the middle of task bar
        ...
        plt.yticks(df['assigned_resource_id'].unique())
